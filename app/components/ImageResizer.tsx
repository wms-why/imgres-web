...
export default function ImageResizer() {
  ...
  const submit = async () => {
    ...
    const total = [...sizes, ...customSizes].map((e) => ({
      scale: e.scale,
      use_ai: e.needAI && e.useAI,
    }));
  ...
  return (
    <section id='tool' className='w-full py-16 bg-gray-100'>
      <div className='max-w-[1024px] mx-auto '>
        <h2 className='text-4xl font-semibold text-center mb-12'>Image Resize Tool</h2>
        <div className='flex flex-col gap-8'>
          ...
          {selectImageErrorShow && <div className='text-red-500 text-xl text-center'>please upload an image first</div>}
          {submitErrorShow && submitErrorMsg && <div className='text-red-500 text-xl text-center'>{submitErrorMsg}</div>}
          {submitErrorShow && !submitErrorMsg && <div className='text-red-500 text-xl text-center'>Image Resize Error, Please Contact Our Team</div>}
        </div>
      </div>
    </section>
  );
